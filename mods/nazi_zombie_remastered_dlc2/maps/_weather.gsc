...
rainHard(transition, targetlvl)
{
	if(!isDefined(targetlvl))
	{
		targetlvl = 9;
	}
...
rainMedium(transition, targetlvl)
{
	if(!isDefined(targetlvl))
	{
		targetlvl = 6;
...
rainLight(transition, targetlvl)
{
	if(!isDefined(targetlvl))
	{
		targetlvl = 2;
...
rainInit(lvl)
{
	...
	{
		iprintln("Unknown raininit type");
		level.rainLevel = 10;
		level._effect["rain_drops"] = level._effect["rain_" + level.rainLevel];
		rainHard(0.1);
	}
...
rainlevelRandom()
{
	...
	{
		//level.rainLevel = 0; // starting rain level
		rainNone(transition_duration);
	}
	else if (lvl < 3)
	{
		//level.rainLevel = lvl;
		rainLight(transition_duration, lvl);
	}
	else if (lvl < 7)
	{
		//level.rainLevel = lvl;
		rainMedium(transition_duration, lvl);
	}
	else if (lvl < 11)
	{
		//level.rainLevel = 10;
		rainHard(transition_duration, lvl);
	}
	else
	{
		iprintln("Unknown rain type");
		//level.rainLevel = 10;
		rainHard(transition_duration);
	}
}
...
rainEffectChange(change, transition)
{
	...
	if (level.rainLevel > change)//Clearing up
	{
		dif = level.rainLevel - change;
		transition /= dif;
		//setdvar("fog_brightness", 0.85);
		for (i=0;i<dif;i++)
		{
			wait (transition);
			level.rainLevel--;
...
player_weather_loop()
{
	...
	for (;;)
	{
		...
		if( level.rainLevel != 0 )
		{
			...
			//PlayFX( level._effect["rain_large_dark_cloud"], self.origin + (0,0,650) );
			self playloopsound("rain_pavement1");//added!!!!
